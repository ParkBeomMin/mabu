...
조달과 제작은 대체재가 아니라 **층이 다르다.** 기성 스킬은 어느 도메인에서나 같은 절차를
담고, 새로 쓰는 스킬은 이 도메인에서만 참인 것(스키마·판별 기준·실패 유형)을 담는다.
아래는 앞 층에 관한 문서다 — 조달을 잘해야 제작할 것이 도메인 지식만 남는다. 사용자 환경에는 이미
...
## 5. 매핑에 있는데 안 깔려 있을 때 — 묻고 고르게 한다

인벤토리에 없다고 곧바로 "원칙 한 줄" 폴백으로 내려가지 마라. **이미 검증된 물건이
존재한다는 사실 자체를 사용자가 모를 수 있다.** 아래 셋을 한 번에 제시하고 고르게 한다.

1. **무엇이 없고 무엇을 잃는지** — 스킬 이름, 어느 에이전트의 어느 자리에 쓸 것인지,
   없을 때 대체되는 원칙 한 줄. §3의 손실 등급을 근거로 쓴다:
   원칙형(`ponytail`)은 거의 무손실, 절차형(`superpowers:*`)·데이터형(`eli5`)은 손실이 크다
2. **설치 명령** — 그대로 복사할 수 있게:
   ```bash
   # 플러그인 (superpowers 등 마켓플레이스에 있는 것)
   claude plugin marketplace add <owner>/<repo>
   claude plugin install <plugin>@<marketplace>

   # 저장소 직배포 스킬 (paperthin 등)
   npx skills@latest add <owner>/<repo> --global --agent claude-code --copy
   ```
   **`--copy`를 뺴지 마라.** 기본은 심볼릭 링크라 자동 업데이트가 되지만, 컨테이너·CI처럼
   원본 경로가 사라지는 환경에서는 스킬이 조용히 죽는다. 그리고 새 스킬은 **다음 세션부터**
   인식된다 — 지금 세션에서 바로 쓰려는 계획을 세우지 마라
3. **안 깔아도 되는 이유** — 폴백으로 진행 가능하며 나중에 깔고 재실행해도 된다는 것

사용자가 거절하면 폴백으로 진행하고, **하네스 CLAUDE.md의 "요구 스킬" 줄에 남긴다**
(§5-1). 나중에 깔았을 때 무엇을 다시 물릴지가 거기 적혀 있어야 한다.

**묻지 않는 경우:** 매핑에 없는 자리, 도메인 고유 지식이라 어차피 새로 써야 하는 자리,
그리고 사용자가 이미 한 번 거절한 스킬. 매번 다시 권하면 설치 영업이 된다.
