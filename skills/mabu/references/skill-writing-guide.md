...
0. [Frontmatter 필드 레퍼런스](#0-frontmatter-필드-레퍼런스)
...
6. [스크립트 번들링 판단 기준](#6-스크립트-번들링-판단-기준) · [6-1 uv + 인라인 의존성](#6-1-파이썬-스크립트는-uv--인라인-의존성으로)
7. [데이터 스키마 표준](#7-데이터-스키마-표준)
8. [스킬에 포함하지 않을 것](#8-스킬에-포함하지-않을-것)

---

## 0. Frontmatter 필드 레퍼런스

SKILL.md 상단 YAML의 공식 필드 (Claude Code v2.1.178+):

| 필드 | 설명 |
|------|------|
| `name` | 표시 이름 (기본값: 디렉터리명) |
| `description` (권장) | 무엇을 하는지 + 언제 쓰는지 — 유일한 자동 트리거 근거 |
| `when_to_use` | description 보조 트리거 구문 |
| `argument-hint` / `arguments` | 슬래시 호출 시 인수 힌트/정의 |
| `disable-model-invocation` | `true`면 자동 트리거 금지 (사용자 슬래시 전용) |
| `user-invocable` | `false`면 사용자 호출 금지 (모델 전용) |
| `allowed-tools` / `disallowed-tools` | 스킬 활성 중 도구 제한 |
| `model` / `effort` | 스킬 활성 중 모델·추론 노력 override |
| `context: fork` + `agent` + `background` | 스킬을 서브에이전트에서 실행 — 무거운 스킬로부터 메인 컨텍스트 보호 |
| `hooks` | 스킬 활성 중 등록할 훅 |
| `paths` | 이 glob에 해당하는 파일 작업 시 활성화 |
| `license` / `compatibility` | Agent Skills 표준 메타 |

자주 쓰는 조합:
- 비싼 일괄 처리 스킬 → `context: fork` (산출물만 돌아오고 과정은 메인 밖)
- 파괴적 작업 스킬 → `disable-model-invocation: true` (사용자가 명시 호출할 때만)
- 검증 전용 스킬 → `allowed-tools: Read, Bash` (수정 도구 차단)
...
| 매번 같은 pip install/npm install 실행 | 의존성을 스크립트에 선언한다 (§6-1) |
| 동일한 다단계 접근법 반복 | 스킬 본문에 표준 절차로 기술 |
| 매번 비슷한 에러 후 같은 회피책 적용 | 스킬에 알려진 문제와 해결법 기술 |

번들링된 스크립트는 반드시 실행 테스트를 거친다.

### 6-1. 파이썬 스크립트는 uv + 인라인 의존성으로

스킬 본문에 "먼저 `pip install X` 하라"고 적지 마라. 그 단계는 에이전트가 건너뛰거나,
가상환경 밖에 깔거나, 이미 깔린 다른 버전과 충돌한다. **의존성을 스크립트 안에 선언하면
설치 단계 자체가 사라진다** — PEP 723 인라인 메타데이터를 `uv`가 읽고 격리된 환경을
알아서 만든다.

```python
# /// script
# requires-python = ">=3.11"
# dependencies = ["httpx", "pillow>=11"]
# ///
import httpx
...
```
...
```bash
uv run scripts/collect.py    # 의존성 해석·설치·실행을 한 번에. 사전 설치 불필요
```

얻는 것:
- **설치 지시가 필요 없다.** 스킬 본문이 짧아지고, 건너뛸 단계가 없어진다
- **환경을 더럽히지 않는다.** 전역 파이썬에 아무것도 안 깔린다
- **의존성의 단일 출처가 파일 자신이다.** 문서와 코드가 갈라지지 않는다
- **휘발성 환경에서 산다.** 컨테이너·CI가 매번 초기화돼도 그대로 돈다
...
노드 스크립트도 원리는 같다 — 전역 설치를 요구하지 말고 `npx <pkg>`로 그 자리에서 돌린다.
